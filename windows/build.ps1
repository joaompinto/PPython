...
function Invoke-Python-Import-Patch
{
    ...
    (Get-Content $file) -replace "python", 'Lib\site-packages\python' | Set-Content $file
    Copy-Item ".\windows\sitecustomize.py" -Destination .\dist\Python-$pythonVersion
}

function Invoke-ReOrg-Files
{

    $keep_files = @("python.cat", "python.exe", "python38.dll", "pythonw.exe", "python38._pth")
    Get-ChildItem -Path ".\dist\Python-$pythonVersion\*" -File -exclude $keep_files | Move-Item -Destination ".\dist\Python-$pythonVersion\Lib\site-packages"
}
